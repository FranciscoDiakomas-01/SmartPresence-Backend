...
export default async function isValidPresence(presence: IPresence) {
  try {
    ...
    const { rowCount: isPendete, rows } = await db.query(
      "SELECT * FROM presence WHERE status = 3 AND teacher_id = $1",
      [id]
    );
    console.log(rows);
    ...
    if (isadm == "teacher") {
      ...
      const { rows } = await db.query(
        "SELECT id , hour_start FROM calendar WHERE week_day = $1 AND teacher_id = $2",
        [today, id]
      );
