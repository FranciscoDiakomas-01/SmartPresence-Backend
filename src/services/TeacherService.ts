...
class TeacherServiceEmplementation {
  ...
  async create(teacher: IUser) {
    ...
    this.#query ="INSERT INTO teacher(name , lastname , email , password , qrcodeurl) VALUES($1 , $2 , $3 , $4 , $5)";
    return new Promise((resolve, reject) => {
      db.query(
        this.#query,
        [teacher.name, teacher.lastname, teacher.email, teacher.password , teacher.qrcode],
        (err, data) => {
          if (err) {
  ...
  async update(teacher: IUser): Promise<string | number> {
    this.#query ="UPDATE teacher SET name = $1 , lastname = $2, email = $3, password = $4 , updated_at = now() WHERE id = $5;";
    return new Promise((resolve, reject) => {
  ...
  async getAll(limit: number, page: number) {
    this.#query =
      "SELECT  id , name , lastname , email , status, qrcodeurl , vocation_date , to_char(created_at, 'YYYY:MM:DD') as created_at FROM teacher OFFSET $1 LIMIT $2;";
  ...
  async toogleStatus(id: number, status: number , date : string = "") {
    this.#query = "UPDATE teacher SET status = $1 , vocation_date = $2  WHERE id = $3;";
  ...
  async login(login: ILogin) {
    this.#query = "SELECT id , password  FROM teacher WHERE email = $1  AND status = 1;";
